...
def generate_sandwich_menu_markdown():
    totals = {"profit": 0, "count": 0}
    codes_sandwiches = {}

    menu = fetch_menu()

    markdown = "## Freshly topped sandwiches\n"
    ...
    markdown += "```markdown\n"
    markdown += format_row(rows[0], col_widths)  # Print header row
    markdown += "\n" + format_separator(col_widths)  # Print separator
    for row in rows[1:]:
        markdown += "\n" + format_row(row, col_widths)  # Print data rows
    markdown += "\n```\n"

    return markdown


def generate_special_menu_markdown():
    totals = {"profit": 0, "count": 0}
    codes_specials = {}

    menu = fetch_menu()

    markdown = "## Special of the Week\n"
    ...
    for product in sorted(menu["products"], key=lambda product: product["price"]):
        ...
        # Only print the special title if we found a valid special
        if row:
            markdown += f"### {title}\n"  # clippy.c_print the special title
            # Prepare data for the Markdown table
            rows = [["Bread Type", "Price"]]
            for bread_type_id in [41, 42, 43, 44, 45]:
                bread_name = menu["breadtypes"][bread_type_id]["name"].title()
            ...
            markdown += "\n" + format_separator(col_widths)  # Print separator
            for r in rows[1:]:
                markdown += "\n" + format_row(r, col_widths)  # Print data rows
            markdown += "\n```\n"

    return markdown


def generate_paninis_menu_markdown():
    totals = {"profit": 0, "count": 0}
    codes_paninis = {}
